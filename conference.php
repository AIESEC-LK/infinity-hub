<!DOCTYPE html>
<html lang="en">
<head>
    <title>INFINITY Hub - AIESEC Sri Lanka</title>
    <meta charset="UTF-8">
    <meta name="description" content="The knowledge Cornerstone of AIESEC Sri Lanka">
    <meta name="keywords" content="event, unica, creative, html">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- Favicon -->
    <link href="img/ico.png" rel="shortcut icon"/>

    <!-- Google Fonts -->
    <link href="https://fonts.googleapis.com/css?family=Rubik:400,400i,500,500i,700,700i" rel="stylesheet">

    <!-- Stylesheets -->
    <link rel="stylesheet" href="css/bootstrap.min.css"/>
    <link rel="stylesheet" href="css/font-awesome.min.css"/>
    <link rel="stylesheet" href="css/themify-icons.css"/>
    <link rel="stylesheet" href="css/magnific-popup.css"/>
    <link rel="stylesheet" href="css/animate.css"/>
    <link rel="stylesheet" href="css/owl.carousel.css"/>
    <link rel="stylesheet" href="css/style.css"/>

</head>
<body>
<?php
require 'common.php';
renderNavbar("", "", "active", "", "")
?>
<div class="site-breadcrumb">
    <div class="container">
        <a href="index.php"><i class="fa fa-home"></i> Home</a> <i class="fa fa-angle-right"></i>
        <span>Conferences & Output</span>
    </div>
</div>
<div class="container">
    <style>
        a {
            margin: 0px;
            -webkit-transition: color 1s;
            transition: color 1s;
        }

        a:hover {
            color: #f6783a;
        }
    </style>
    <div class="row">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <style>
            .btn {
                border: 2px solid black;
                background-color: white;
                color: black;
                padding: 14px 28px;
                font-size: 14px;
                cursor: pointer;
                width: 240px;
                height: 50px;
                margin: 10px;
                margin-bottom: 50px;
            }

            /* Green */
            .success {
                border-color: #0e8ea0;
                color: #0e8ea0;
            }

            .success:hover {
                background-color: #0e8ea0;
                color: white;
            }

        </style>
                <button class="btn success" onclick="location.href='#eps2022';" align="center">EPS 2022</button>

        <button class="btn success" onclick="location.href='#bdsummit2022';" align="center">BD summit 2022</button>
        <button class="btn success" onclick="location.href='#excon2022';" align="center">ExCon 2022</button>
        <button class="btn success" onclick="location.href='#NatCon22';" align="center">NatCon 2022</button>
        <button class="btn success" onclick="location.href='#ic21';" align="center">IC 2021</button>
        <button class="btn success" onclick="location.href='#nlds21';" align="center">NLDS 2021</button>
        <button class="btn success" onclick="location.href='#impactco20';" align="center">Impactco 2020</button>
        <button class="btn success" onclick="location.href='#apc2020';" align="center">APC 2020</button>
        <button class="btn success" onclick="location.href='#nlds2019';" align="center">NLDS 2019</button>
        <button class="btn success" onclick="location.href='#apc2019';" align="center">APC 2019</button>

    </div>
</div>
</section>
</div>
</section>
<section class="blog-page-section spad pt-0">
    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="post-item post-details">
                    <img src="img/eps2022.jpg" width="110%" style="height: 400px" class="post-thumb-full">
                    <div class="post-content">
                        <h3><a href="single-blog.html"></a></h3>
                        <div class="post-meta">
                        </div>
                        <div class="tag"></div>
                    </div>
                    <div class="post-author">
                    </div>
                </div>
            </div>
            <div class="col-sm-8 col-md-5 col-lg-4 col-xl-3 offset-xl-1 offset-0 pl-xl-0 sidebar">
                <div class="widget" name="eps2022" id="eps2022" alt="">
                    &nbsp
                    <h4 class="widget-title">Expansion Planning Summit 2022</h4>
                    <p>The Expansion Planning summit 2022 took place from 27th and 28th of August 2022 in order to redefine the roadmap of initiative groups of AIESEC in Sri Lanka.
The National Expansion Board along with the member committee, facilitated discussions, evaluations, and thereby the implementations for the upcoming term. The summit was held in a physical context with interactive spaces while empowering Expansion Development.</p>
                </div>
                <div class="widget">
                    <h4 class="widget-title"></h4>
                    <div class="tags">
                        <a href="https://drive.google.com/drive/folders/1dboB2NIf5uukqnKPHambwJ4W7a3wW79q?usp=share_link">Output</a>
                    </div>
                </div>

            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="post-item post-details">
                    <img src="img/conf/busimmit.JPG" width="110%" style="height: 400px" class="post-thumb-full">
                    <div class="post-content">
                        <h3><a href="single-blog.html"></a></h3>
                        <div class="post-meta">
                        </div>
                        <div class="tag"></div>
                    </div>
                    <div class="post-author">
                    </div>
                </div>
            </div>
            <div class="col-sm-8 col-md-5 col-lg-4 col-xl-3 offset-xl-1 offset-0 pl-xl-0 sidebar">
                <div class="widget" name="bdsummit2022" id="bdsummit2022" alt="">
                    &nbsp
                    <h4 class="widget-title">BD Summit 2022</h4>
                    <p> The BD summit held on September 2022 organized by AIESEC in Sri Lanka, focused on giving away knowledge on BD sustainability and account management.</p>
                </div>
                <div class="widget">
                    <h4 class="widget-title"></h4>
                    <div class="tags">
                        <a href="https://drive.google.com/drive/folders/1XBlBsD3mCFcQ4psgRN9WiYL9imI4XBJ1?usp=sharing">Day
                            1</a>
                    </div>
                </div>

            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="post-item post-details">
                    <img src="img/conf/excon2022.jpg" width="110%" style="height: 400px" class="post-thumb-full">
                    <div class="post-content">
                        <h3><a href="single-blog.html"></a></h3>
                        <div class="post-meta">
                        </div>
                        <div class="tag"></div>
                    </div>
                    <div class="post-author">
                    </div>
                </div>
            </div>
            <div class="col-sm-8 col-md-5 col-lg-4 col-xl-3 offset-xl-1 offset-0 pl-xl-0 sidebar">
                <div class="widget" name="excon2022" id="excon2022" alt="">
                    &nbsp
                    <h4 class="widget-title">ExCon 2022</h4>
                    <p> ExCon is the Exclusive Initiative Group National Conference by AIESEC Sri Lanka.
                        It's a platform for the Initiative Group Membership to have a holistic understanding of AIESEC Sri Lanka, make them aware of the core of AIESEC,
                        empower them to become value-driven leaders and, most importantly, to make lifelong friend.</p>
                </div>
                <div class="widget">
                    <h4 class="widget-title"></h4>
                    <div class="tags">
                        <a href="https://drive.google.com/drive/folders/1-wbYjzrwaAh8I7jsT6WgLxCIVvesYTfU?usp=sharing">Day
                            1</a>
                        <a href="https://drive.google.com/drive/folders/1eTYPHbsvm1lkJmV9lOl1wQjD3OyaXP84?usp=sharing ">Day
                            2</a>
                    </div>
                </div>

            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="post-item post-details">
                    <img src="img/conf/Natcon.png" width="110%" style="height: 400px" class="post-thumb-full">
                    <div class="post-content">
                        <h3><a href="single-blog.html"></a></h3>
                        <div class="post-meta">
                        </div>
                        <div class="tag"></div>
                    </div>
                    <div class="post-author">
                    </div>
                </div>
            </div>
            <div class="col-sm-8 col-md-5 col-lg-4 col-xl-3 offset-xl-1 offset-0 pl-xl-0 sidebar">
                <div class="widget" name="NatCon22" id="NatCon22" alt="">
                    &nbsp
                    <h4 class="widget-title">NatCon 2022</h4>
                    <p>NatCon is the largest conference in AIESEC Sri Lanka . It paves the way for you to find the hidden leader in you while enjoying yourself with your
                        virtually met friends  and whole new friends who are from different entities of AIESEC .
                        The conference will primarily focus on strengthening AIESEC membership understanding and empowering
                        individuals to discover the true leader inside themselves.It will be a life-changing experience for you, in which you will recognize yourself
                        as a young leader who is transforming the world and fulfilling AIESEC’s vision.</p>
                </div>
                <div class="widget">
                    <h4 class="widget-title"></h4>
                    <div class="tags">
                        <a href="https://drive.google.com/drive/folders/1xqMONZWwNq6bpFFM6Ij_dgiJNxyC1ayR?usp=sharing">Day
                            1</a>
                        <a href="https://drive.google.com/drive/folders/1dtsMpwWCZC4INrX9lxUUXLXtEdYxMn8p?usp=sharing">Day
                            2</a>
                        <a href="https://drive.google.com/drive/folders/1KJN05B4hN6GqikhWvG_t2g6a4WktlzCC?usp=sharing">Day
                            3</a>
                    </div>
                </div>

            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="post-item post-details">
                    <img src="img/conf/ic21.png" width="110%" style="height: 400px" class="post-thumb-full">
                    <div class="post-content">
                        <h3><a href="single-blog.html"></a></h3>
                        <div class="post-meta">
                        </div>
                        <div class="tag"></div>
                    </div>
                    <div class="post-author">
                    </div>
                </div>
            </div>
            <div class="col-sm-8 col-md-5 col-lg-4 col-xl-3 offset-xl-1 offset-0 pl-xl-0 sidebar">
                <div class="widget" name="ic21" id="ic21" alt="">
                    &nbsp
                    <h4 class="widget-title">IC - International Congress 2021</h4>
                    <p>AIESEC International Congress is an annual occasion where the organisational state of affairs are
                        reviewed,
                        new goals are set, views and experience are shared, which are focused on catering to the
                        interest of global youth and world issues.</p>
                </div>
                <div class="widget">
                    <h4 class="widget-title"></h4>
                    <div class="tags">
                        <a href="https://drive.google.com/drive/folders/1IIhFN3UBQn05yf2v6RSAyK-UXDv_uupA?usp=sharing">Day
                            0</a>
                        <a href="https://drive.google.com/drive/folders/1AEh22JFILbUNUnUvDunB7T1Kgdjl90I8?usp=sharing">Day
                            1</a>
                        <a href="https://drive.google.com/drive/folders/1gzq-x_Qa4MHLr_loNEfyth2xh0crhe4g?usp=sharing">Day
                            2</a>
                        <a href="https://drive.google.com/drive/folders/1_4lWiNytWshzsmgqYB7uUSI-vN6sMelS?usp=sharing">Day
                            3</a>
                        <a href="https://drive.google.com/drive/folders/1hts3IBbY34iQwn42b-wf8BFoJPvwFFsI?usp=sharing">Day
                            4</a>
                        <a href="https://drive.google.com/drive/folders/180_FCIM4OQavIKorRltVrluD3LgLwyv5?usp=sharing">Day
                            5</a>
                        <a href="https://drive.google.com/drive/folders/1sBE-kfK76HyZs4sWHEHhn2SVpWNvyetD?usp=sharing">Day
                            6</a>
                    </div>
                </div>

            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="post-item post-details">
                    <img src="img/conf/nlds2021.png" width="110%" style="height: 400px" class="post-thumb-full">
                    <div class="post-content">
                        <h3><a href="single-blog.html"></a></h3>
                        <div class="post-meta">
                        </div>
                        <div class="tag"></div>
                    </div>
                    <div class="post-author">
                    </div>
                </div>
            </div>
            <div class="col-sm-8 col-md-5 col-lg-4 col-xl-3 offset-xl-1 offset-0 pl-xl-0 sidebar">
                <div class="widget" name="nlds21" id="nlds21" alt="">
                    &nbsp
                    <h4 class="widget-title">NLDS - National Leadership Development Seminar 2021</h4>
                    <p>NLDS is the most anticipated national conference of the year conducted to develop leadership,
                        enhance employability and personal development skills of AIESECers. This three-day conference
                        connects 250+ AIESECers through events, workshops and fun activities.</p>
                </div>
                <div class="widget">
                    <h4 class="widget-title"></h4>
                    <div class="tags">
                        <a href="https://drive.google.com/drive/folders/1DT7drDOus2VX5gmBft5F-ClISalEHkBB?usp=sharing">Day
                            1</a>
                        <a href="https://drive.google.com/drive/folders/1uvDJJWazwc0O8nmbqTmvXS9-xuIrIqdT?usp=sharing">Day
                            2</a>
                        <a href="https://drive.google.com/drive/folders/1mDanEOurPMTzVo_f76e5I71NzhFWrScs?usp=sharing">Day
                            3</a>
                    </div>
                </div>

            </div>
        </div>
    </div>

    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="post-item post-details">
                    <img src="img/conf/impactco2020.png" width="110%" style="height: 400px" class="post-thumb-full">
                    <div class="post-content">
                        <h3><a href="single-blog.html"></a></h3>
                        <div class="post-meta">
                        </div>
                        <div class="tag"></div>
                    </div>
                    <div class="post-author">

                    </div>

                </div>
            </div>
            <div class="col-sm-8 col-md-5 col-lg-4 col-xl-3 offset-xl-1 offset-0 pl-xl-0 sidebar">
                <div class="widget" name="impactco20" id="impactco20" alt="">
                    &nbsp
                    <h4 class="widget-title">Impactco - Impact Conference 2020</h4>
                    <p>Impactco 2020 is the first ever virtual conference in AIESEC Sri Lanka. This conference will be
                        held on December 11, 2020 to December 13, 2020. </p>
                </div>
                <div class="widget">
                    <h4 class="widget-title"></h4>
                    <div class="tags">
                        <a href="https://drive.google.com/drive/folders/10BH1Nd0ogFWynLxyiC7nA2a8Eurr6Z7J?usp=sharing">Day
                            1</a>
                        <a href="https://drive.google.com/drive/folders/1jnhjRnTXgQkcp_MngCP5UNDeuUxzyywT?usp=sharing">Day
                            2</a>
                        <a href="https://drive.google.com/drive/folders/1zhryix1qrBeA2OlpXV0ThviOHKCrpfNb?usp=sharing">Day
                            3</a>
                    </div>
                </div>

            </div>
        </div>
    </div>

    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="post-item post-details">
                    <img src="img/blog/apc20.jpg" width="110%" style="height: 400px" class="post-thumb-full">
                    <div class="post-content">
                        <h3><a href="single-blog.html"></a></h3>
                        <div class="post-meta">
                        </div>
                        <div class="tag"></div>
                    </div>
                    <div class="post-author">

                    </div>

                </div>
            </div>
            <div class="col-sm-8 col-md-5 col-lg-4 col-xl-3 offset-xl-1 offset-0 pl-xl-0 sidebar">
                <div class="widget" name="apc2020" id="apc2020" alt="">
                    &nbsp
                    <h4 class="widget-title">Asia Paciffic Conference 2020 Output</h4>
                    <p>Asia Pacific Confernce is a regional conference which was held on March 25, 2020 to March 29,
                        2020. This is a virtual coference and happend for a period of 5 days.</p>
                </div>
                <div class="widget">
                    <h4 class="widget-title"></h4>
                    <div class="tags">
                        <a href="https://sites.google.com/view/wearelimitless/home">Conference Output</a>
                    </div>
                </div>

            </div>
        </div>
    </div>

    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="post-item post-details">
                    <img src="img/blog/11.jpg" width="100%" class="post-thumb-full" alt="">
                    <div class="post-content">
                        <h3><a href="single-blog.html"></a></h3>
                        <div class="post-meta">
                        </div>
                        <div class="tag"></div>
                    </div>
                    <div class="post-author">

                    </div>

                </div>
            </div>
            <div class="col-sm-8 col-md-5 col-lg-4 col-xl-3 offset-xl-1 offset-0 pl-xl-0 sidebar">
                <div class="widget" name="nlds2019" id="nlds2019">
                    <h4 class="widget-title">National Leadership Development Seminar 2019 Output</h4>
                    <p>National Leadership Development Seminar was held on OCT 24, 2019 AT 8 AM – OCT 27, 2019 at
                        Carolina Beach Resort & Spa.</p>
                </div>

                <div class="widget">
                    <h4 class="widget-title">Days</h4>
                    <div class="tags">
                        <a href="https://drive.google.com/drive/folders/1IY1_o99yarizzPDOyEJ20B4ik7fpphHS?usp=sharing">Day
                            01</a>
                        <a href="#">Day 02</a>
                        <a href="#">Day 03</a>
                        <a href="#">Day 04</a>
                    </div>
                </div>

            </div>
        </div>
    </div>

    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="post-item post-details">
                    <img src="img/blog/1.jpg" width="110%" style="height: 400px" class="post-thumb-full">
                    <div class="post-content">
                        <h3><a href="single-blog.html"></a></h3>
                        <div class="post-meta">


                        </div>

                        <div class="tag"></div>
                    </div>
                    <div class="post-author">

                    </div>

                </div>
            </div>
            <div class="col-sm-8 col-md-5 col-lg-4 col-xl-3 offset-xl-1 offset-0 pl-xl-0 sidebar">
                <div class="widget" name="apc2019" id="apc2019" alt="">
                    <h4 class="widget-title">Asia Paciffic Conference 2019 Output</h4>
                    <p>Asia Pacific Confernce is a regional conference which was held on March 24, 2019 to March 28,
                        2019 in Thailand through out a period of 5 days time.</p>
                </div>
                <div class="widget">
                    <h4 class="widget-title">Days</h4>
                    <div class="tags">
                        <a href="https://drive.google.com/drive/folders/1-tA5LQ8mat-v1cP5Wn4yXn5Xh8aAYfCo?usp=sharing">AI
                            Message</a>
                        <a href="https://drive.google.com/drive/folders/15xXpLlEGngXWhSJRZniKz81zfHzAiZEA?usp=sharing">Day
                            01</a>
                        <a href="https://drive.google.com/drive/folders/1DsF7Kk3XXjWKo8xeId5ObF2eSyxu4A10?usp=sharing">Day
                            02</a>
                        <a href="https://drive.google.com/drive/folders/1SaZZ1Zy6pdeniOLu6v4GuyihN0-JQZXY?usp=sharing">Day
                            03</a>
                        <a href="https://drive.google.com/drive/folders/1N-Wn8mHl05R3XszUHvz0KhnpNUqc2K5P?usp=sharing">Day
                            04</a>
                        <a href="https://drive.google.com/drive/folders/19dW1K0B8uOVlHeLub2XIo9lzWkIQNnor?usp=sharing">Day
                            05</a>
                    </div>
                </div>

            </div>
        </div>
    </div>
</section>

<?php
renderFooter();
?>

<!--====== Javascripts & Jquery ======-->
<script src="js/jquery-3.2.1.min.js"></script>
<script src="js/owl.carousel.min.js"></script>
<script src="js/jquery.countdown.js"></script>
<script src="js/masonry.pkgd.min.js"></script>
<script src="js/magnific-popup.min.js"></script>
<script src="js/main.js"></script>
<script src="https://code.jquery.com/jquery-3.4.1.slim.min.js"
        integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n"
        crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js"
        integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo"
        crossorigin="anonymous"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js"
        integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6"
        crossorigin="anonymous"></script>

</body>
</html>